...
Phase 1: Backend Setup & Configuration (Strapi)
 Initialize a new Strapi v5 project (backend folder).
 Configure Strapi to connect to Neon.tech PostgreSQL database.
 Integrate Strapi Cloudinary provider plugin for free image storage.
 Set up Data Modeling (Event, Category, RSVP).
 Set up Strapi Roles & Permissions.
 Expose REST API.
Phase 2: Frontend Foundation & Caching (Next.js + Redis)
 Initialize Next.js 16+ project (frontend folder) with Tailwind CSS.
 Setup Shadcn/ui.
 Configure environment variables.
 Create caching utility (Upstash Redis).
 Create core layout.
Phase 3: Core Features (Map & List Views)
 Integrate interactive map (Leaflet.js).
 Build Event List View.
 Implement synchronization between map and list.
 Add Category Filtering & Search.
Phase 4: Event Details & Media
 Create dynamic Event Detail Page.
 Build Featured Events Section carousel.
Phase 5: Authentication & User Engagement
 Auth Integration (NextAuth.js + Strapi).
 Role Management.
 RSVP System.
 User Dashboard.